...
# --- Per-user credentials for admin-defined connections ---------------------
# The admin declares which slots a connection needs; each user fills in their own
# values. Nothing here lets a user reach a connection, a header, or a value that
# the admin did not declare.

USER_KEY_AUTH_TYPE = 'user_key'
USER_KEY_SLOT = 'key'
USER_KEY_PROPERTY = {
    'type': 'string',
    'title': 'API Key',
...
SLOT_NAME_PATTERN = re.compile(r'^[A-Za-z0-9_\-]{1,64}$')
MAX_USER_CONFIG_VALUE_LENGTH = 4096


class ToolServerUserConfigRequiredError(Exception):
    """A connection needs per-user credentials the user has not provided yet."""

    def __init__(self, server_id: str, slots: list[str]):
        self.server_id = server_id
        self.slots = slots
        super().__init__(
            f"Missing credentials for tool server '{server_id}': {', '.join(slots)}. "
