<script lang="ts">
	...
	// Gallery — persistent images from backend
	...
	const submitHandler = async () => {
		...
		try {
			...
			if (result) {
				// Pull the new images back from the server rather than synthesising
				// entries locally: the engine decides the model actually used and may
				// adjust the requested dimensions, so the stored metadata is the only
				// accurate source for the gallery's labels.
	...
	/**
	 * Resolve which of the given images still have a reachable source chat, so
	 * "Open in Chat" is only offered when it leads somewhere.
	 *
	 * Pass `recheck` to re-test chats that already resolved: a chat deleted
	 * elsewhere stays cached as valid otherwise, leaving a dead link behind.
	 */
	const syncChatLinks = async (files: GalleryImageFile[], recheck = false) => {
		const chatIds = new Set<string>();
		for (const f of files) {
			const chatId = (f.meta?.data as Record<string, unknown>)?.chat_id as string;
			if (chatId && (recheck || !validChatIds.has(chatId))) {
				chatIds.add(chatId);
			}
		}
		...
		await Promise.all(
			[...chatIds].map(async (id) => {
				try {
					...
					// Deleted or no longer accessible — drop any link we were showing.
					validChatIds.delete(id);
				}
			})
		);
		validChatIds = validChatIds; // trigger reactivity
	};
	...
	const loadGalleryPage = async () => {
		...
		try {
			...
			if (files && files.length > 0) {
				// Advance by what the server returned, not by a page index: images
				// prepended by refreshGallery() shift the server's list, and appending
				// an image already on screen would duplicate a key in the {#each}.
			...
			// Surface it: silently showing an empty gallery would be indistinguishable
			// from genuinely having no images.
			galleryHasMore = false;
			toast.error(`${error}`);
		}
	...
	/**
	 * Pick up images generated outside this view and re-resolve the chat links.
	 *
	 * Only runs when the browser tab regains visibility, so it does not catch
	 * changes made side-by-side in the same tab — reopening the tab or reloading
	...
	const refreshGallery = async () => {
		...
		try {
			...
			// Find images we don't already have by ID
			const existingIds = new Set(galleryImages.map((f) => f.id));
			const newFiles = latestFiles.filter((f: GalleryImageFile) => !existingIds.has(f.id));

			if (newFiles.length > 0) {
				galleryImages = [...newFiles, ...galleryImages];
				// Those rows now sit ahead of everything already loaded.
				galleryOffset += newFiles.length;
			}

			// Re-test every chat we are linking to, not just the new arrivals, so
			// chats deleted since the last check stop offering "Open in Chat".
			await syncChatLinks(galleryImages, true);
		} catch {
			// Silently ignore refresh errors — gallery is still usable
		}
	};
	...
	const deleteGalleryImage = async (fileId: string) => {
		try {
			...
			// One fewer row on the server, so the next page starts one earlier.
			galleryOffset = Math.max(0, galleryOffset - 1);
			toast.success($i18n.t('Image deleted'));

			// If all visible images were deleted but more exist, reset and reload
	...
	onMount(async () => {
		...
		// Listen for tab visibility changes to refresh gallery
		// (picks up images generated in other tabs, e.g. from chat)
								...
								<div class="grid gap-3 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
									{#each galleryImages as file (file.id)}
										...
										>
											...
											<div
												class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity rounded-xl flex items-end justify-center pb-3"
											>
												<div
